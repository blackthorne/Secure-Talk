...
void * send_others(void *ptr){
	...
	while(fgets(input, MAX_INPUT_BUF + 1, stdin) != NULL){
		...
		if(	input_size > newkey_cmd_size &&  // just for performance
			...
			if(proto_renew_key(&pctx, key, KEY_SIZE) == 0 && VERBOSE_MODE)
...
void * listen_others(){
	...
	while(1)
		if(proto_recv_msg(&pctx, s_ip, &s_port, msg, &size) == 0){ // past -> && (pctx.smsg.size < MAX_RECV_BUF - (NONCE_FIELD_SIZE + sizeof(int) + 1 + SHA_DIGEST_LEN))){
